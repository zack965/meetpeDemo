...
export interface Guide {
    ...
    name: string
    email: string
    hamiltong_distance: number
    profile_path: string
    choices_guide: string[]


}
export interface SqlMatcingGuide {
    id: number
    name: string
    email: string
    cnt: number
...
export interface CosignSimularityGuide {
    id: number
    name: string
    email: string
    value: number
...
function HamiltongDistanceMatching() {
    ...
    useEffect(() => {
        ...
        axios.get<Choices>("http://127.0.0.1:8000/api/getChoices", {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'api_key': 'itHP54kdcSKkO1IgbTnm6jZdsjmKTj4VsHNA4eLDP2N84hGdJCI59ynr6bSKK5Pv',
    ...
    const matchByVoyageur = (user_id: number) => {
        axios.post<Guide[]>(`http://127.0.0.1:8000/api/MatchingHammingDistance/${user_id}`, null, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'api_key': 'itHP54kdcSKkO1IgbTnm6jZdsjmKTj4VsHNA4eLDP2N84hGdJCI59ynr6bSKK5Pv',
                ...
                <div className="w-1/2">
                    {MatchedGuides.length == 0 ? (<div className="flex w-full h-screen justify-center items-center">
                        ...
                        {MatchedGuides.map((voyageur) => (
                            ...
                            <div className="bg-gray-50 mx-10 px-10 py-2 my-2 rounded-lg flex justify-between">
                                <p>
                                    {voyageur.name} : {voyageur.hamiltong_distance}
                                </p>
...
export default HamiltongDistanceMatching
